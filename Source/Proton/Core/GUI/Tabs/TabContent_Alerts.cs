...
namespace Proton
{
    public class TabContent_Alerts : ITabContent
    {
        ...
        private readonly Listing_Collapsible selection_collapsible = new Listing_Collapsible();
        ...
        public TabContent_Alerts()
        {
            if (descriptionBoxes == null)
            {
                ...
                {
                    new Pair<Color, string>(Color.green, "Proton.Colored.ActiveNow".Translate()),
                    new Pair<Color, string>(Color.yellow, "Proton.Colored.IgnoredAndBad".Translate()),
                    new Pair<Color, string>(Color.red, "Proton.Colored.DisabledOrBad".Translate()),
                    new Pair<Color, string>(Color.blue, "Proton.Colored.Ignored".Translate())
                };
            }
        }
        ...
        public override void DoContent(Rect rect)
        {
            ...
            if (collapsible.CheckboxLabeled(KeyedResources.Proton_AlertsDisabled,
                    ref RocketPrefs.DisableAllAlert, disabled: !RocketPrefs.AlertThrottling))
            {
                RocketMod.Instance.WriteSettings();
            ...
            else
            {
                MissileGirl.GUIUtility.ExecuteSafeGUIAction(() =>
                {
                    ...
                    Widgets.Label(rect, RocketPrefs.DisableAllAlert
                        ? "Proton.Disabled".Translate()
                        : "Proton.AlertsDisabled".Translate());
                });
            }
            ...
            MissileGirl.GUIUtility.ExecuteSafeGUIAction(() =>
            {
                ...
                MissileGirl.GUIUtility.Row(curRect.BottomHalf(), new List<Action<Rect>>
                {
                    tempRect =>
                    {
                        ...
                        Widgets.TextFieldNumeric(tempRect, ref Context.Settings.executionTimeLimit, ref buffer1, 1.0f,
                            100.0f);
                    },
                    tempRect =>
                    {
        ...
        private void DoScrollView(Rect inRect)
        {
            MissileGirl.GUIUtility.ExecuteSafeGUIAction(() =>
            {
                ...
                if (curAlert != null)
                {
                    MissileGirl.GUIUtility.ExecuteSafeGUIAction(() =>
                    {
                        ...
                        selection_collapsible.Begin(inRect,
                            KeyedResources.MissileGirl_Selection.Formatted(curAlert.GetName()), drawIcon: false,
                            drawInfo: false);
                        if (selection_collapsible.CheckboxLabeled("Proton.Enabled".Translate() + "</color>",
                                ref curSettings.enabledInt))
                        {
                            curSettings.UpdateAlert(true);
                            RocketMod.Instance.WriteSettings();
                        }
                        if (selection_collapsible.CheckboxLabeled("Proton.IgnoreThis".Translate(),
                                ref curSettings.ignored))
                        {
                            curSettings.UpdateAlert(true);
                            RocketMod.Instance.WriteSettings();
                        }
            ...
            MissileGirl.GUIUtility.ExecuteSafeGUIAction(() =>
            {
                ...
                Widgets.DrawMenuSection(curRect);
                GUIFont.Font = GUIFontSize.Tiny;
                MissileGirl.GUIUtility.GridView(curRect, 2, descriptionBoxes,
                    (descriptionRect, pair) => MissileGirl.GUIUtility.ColorBoxDescription(descriptionRect, pair.first,
                        pair.second), drawBackground: false);
            });
            inRect.yMin += 45;
            ...
            MissileGirl.GUIUtility.ExecuteSafeGUIAction(() =>
            {
                ...
                tempRect.xMin += 10;
                tempRect.xMax -= 25;
                MissileGirl.GUIUtility.GridView(tempRect.TopPartPixels(25), 3, new List<Action<Rect>>
                {
                    curRect => Widgets.Label(curRect, "Proton.AlertName".Translate()),
                    curRect => Widgets.Label(curRect, "Proton.Avg".Translate()),
                    curRect => Widgets.Label(curRect, "Proton.TimeSinceLast".Translate())
                }, (gridRect, action) => action.Invoke(gridRect), drawBackground: false);
            });
            inRect.yMin += 25;
            ...
            MissileGirl.GUIUtility.ScrollView(inRect, ref scrollPosition, Context.ReadoutInstance.AllAlerts,
                alert =>
                {
                    if (alert == null || !Context.AlertToSettings.ContainsKey(alert))
                        return -1f;
                    if (searchString.NullOrEmpty())
                        return 35f;
                    return alert.GetNameLower().Contains(searchString) ? 40f : -1f;
                },
                (alertRect, alert) =>
                {
                    AlertSettings settings = Context.AlertToSettings[alert];
                    if (settings.AverageExecutionTime > Context.Settings.executionTimeLimit)
                        Widgets.DrawBoxSolid(alertRect, settings.ignored ? warningColor : dangerColor);

                    if (Widgets.ButtonInvisible(alertRect))
                    {
                        curAlert = alert;
                        curSettings = settings;
                    }

                    Widgets.DrawBoxSolid(alertRect.LeftPartPixels(3), !settings.ignored
                        ? settings.enabledInt
                            ? alert.cachedActive ? Color.green : Color.grey
                            : Color.red
                        : Color.blue);

                    MissileGirl.GUIUtility.GridView(alertRect, 3, new List<Action<Rect>>
                    {
                        curRect =>
                        {
                            curRect.xMin += 3;
                        ...
                        curRect => Widgets.Label(curRect, $"{Math.Round(settings.AverageExecutionTime, 3)} MS"),
                        curRect => Widgets.Label(curRect,
                            $"{Math.Round(settings.TimeSinceLastExecution, 3)} Seconds")
                    }, (gridRect, action) => action.Invoke(gridRect), drawBackground: false);
                });
        }
